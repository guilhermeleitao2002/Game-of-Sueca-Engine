...
class CooperativePlayer(Player):
    ...
    def __init__(self, name, team) -> None:
        ...
        self.partners_deck = np.ones(40)
        for card in self.hand:
            self.partners_deck[self.card_ordering_index[card.name]] = 0

    def update_partners_deck(self, cards_played: list[Card], round_suit: str) -> None:
        '''
            Updates the prediction of partners deck
        '''
        for card in cards_played:
            self.partners_deck[self.card_ordering_index[card.name]] = 0

        if len(cards_played) >= 2:
            partner_card = cards_played[-2].suit

            if partner_card.suit != round_suit:
                print(f"Partner doesn't have any more {round_suit}!")
                match round_suit:
                    case "hearts":
                        j = 0
                    case "diamonds":
    ...
    def play_round(self, i, cards_played, round_suit) -> Card:
        ...
        '''
        self.update_partners_deck(cards_played, round_suit)
        print("" + cards_played)
        if i == 0:
            # Check for the user's cards if the partner is able to win the round
            # If None is good, play a card that doesn't loose points
            winning_cards = []
            for card in self.hand:
                pass
